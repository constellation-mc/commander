...
public class DynamicRegistryRegistryFunction extends AbstractFunction {
    ...
    public EvaluationValue evaluate(EvaluationContext context, Token functionToken, EvaluationValue... par) throws EvaluationException {
        try {
            ...
            return ReflectiveValueConverter.convert(server.getRegistryManager().getOptional(RegistryKey.ofRegistry(id)).orElseThrow(() -> new NoSuchElementException("No such %s: %s".formatted("minecraft:root", id))));
        } catch (Exception e) {
