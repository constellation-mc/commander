...
public class DynamicRegistryFunction extends AbstractFunction {
    ...
    public EvaluationValue evaluate(EvaluationContext context, Token functionToken, EvaluationValue... par) throws EvaluationException {
        try {
            ...
            return ReflectiveValueConverter.convert(server.getRegistryManager().get(registry).getOrEmpty(id).orElseThrow(() -> new NoSuchElementException("No such %s: %s".formatted(registry.getValue(), id))));
        } catch (Exception e) {
