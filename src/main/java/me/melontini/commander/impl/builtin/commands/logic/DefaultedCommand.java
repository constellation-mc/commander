...
public record DefaultedCommand(List<Command.Conditioned> commands, Optional<Command.Conditioned> then) implements Command {

    public static final Codec<DefaultedCommand> CODEC = RecordCodecBuilder.create(data -> data.group(
            ExtraCodecs.list(Command.CODEC).fieldOf("commands").forGetter(DefaultedCommand::commands),
            ExtraCodecs.optional("then", Command.CODEC).forGetter(DefaultedCommand::then)
