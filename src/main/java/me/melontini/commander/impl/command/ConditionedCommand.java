...
public record ConditionedCommand(Optional<LootCondition> condition, Command other) implements Command.Conditioned {
    ...
    public static final MapCodec<? extends Command.Conditioned> CODEC = new MapCodec<ConditionedCommand>() {
        ...
        public <T> RecordBuilder<T> encode(ConditionedCommand input, DynamicOps<T> ops, RecordBuilder<T> prefix) {
            ...
            input.condition().map(condition1 -> LootCondition.CODEC.encodeStart(ops, condition1)).ifPresent(tDataResult -> r.add("condition", tDataResult));
            return r;
        }
        ...
        public <T> DataResult<ConditionedCommand> decode(DynamicOps<T> ops, MapLike<T> input) {
            ...
            return r.map(command -> LootCondition.CODEC.parse(ops, condition).map(condition1 -> new ConditionedCommand(Optional.of(condition1), command))).flatMap(Function.identity());
        }
